...
case <- readRDS("dat/case.rds")
names(case)
control <- readRDS("dat/control.rds")
names(control)
...
df <- data.frame(day.diff <- c(5, 10, 15, 20, 25, 31), 
                 pairs <- c(1281, 1743, 2042, 2222, 2351, 2477)
                 )
...
save(case.m, control.m, id.lookup1, file = "dat/case_control_matched.rda")
write.csv(case.m, file = "dat/case_matched.csv", row.names = F)
write.csv(control.m, file = "dat/comtrol_matched.csv", row.names = F)
...
library(ggplot2)
tiff(file = "fig/barplot_case_control_pairs.tiff", width = 10, height = 6, units = "in", res = 300)
...
#   mutate(birthYMD = as.Date(ISOdate(birthY, birthM, 1)))
# 
# plot.control <- ggplot(y1, aes(x = birthYMD, y = n)) +
#   geom_bar(stat = "identity") +
#   scale_x_date(date_breaks = "1 year", date_labels = "%Y") +
#   labs(x = "Year", y = "No. of controls") +
#   theme_publication()
# 
# tiff(file = "barplot_matched_case_control.tiff", width = 10, height = 6, units = "in", res = 300)
# library(cowplot)
# plot_grid(plot.case, plot.control, labels = c("A", "B"), nrow = 2, align = "v")
# dev.off()
...
save(logitmod1, file = "logitmod1.rda")
...
save(cv.modpls.logit, file = "cv.modpls.logit.rda")
res.cv.modpls.logit <- cvtable(summary(cv.modpls.logit, MClassed = TRUE))
plot(res.cv.modpls.logit)

res <- plsRglm(dataY = diagnosis, dataX = x, nt = 3, modele = "pls-glm-logistic", 
               pvals.expli = TRUE)
res$wwetoile
...
# the number of repetitions must be larger than the number of data rows, otherwise 
# it will produce following error:
# Error in bca.ci(boot.out, conf, index[1L], L = L, t = t.o, t0 = t0.o,  : 
# estimated adjustment 'a' is NA
CHD.bootYX3 <- bootplsglm(res, typeboot = "plsmodel", R = 10000)
boxplots.bootpls(CHD.bootYX3, las = 2, mar = c(8, 2, 1, 1) + 0.1)
temp.ci <- confints.bootpls(CHD.bootYX3)
plots.confints.bootpls(temp.ci, typeIC = "BCa", colIC = c("blue", "blue", "blue", "blue"),
                       legendpos = "topright", las = 2, mar = c(5, 2, 1, 1) + 0.1)
save(CHD.bootYX3, temp.ci, file = "model.rda")
...
data <- data %>%
  mutate_all(factor) %>%
  mutate(F.production.age = as.integer(F.production.age),
         M.production.age = as.integer(M.production.age))
inTrain <- createDataPartition(y = data$diagnosis, p = .75, list = FALSE)
train.data <- data[inTrain, ]
test.data <- data[-inTrain, ]

table(train.data$diagnosis)
table(test.data$diagnosis)

# balancing by sampling stratification
nRareSamples <- length(which(train.data$diagnosis == 0))
rf.strata <- randomForest(diagnosis ~ ., data = train.data, 
                          strata = train.data$diagnosis, 
...
varImp(rf.strata)
par(mfrow = c(1, 1), mar = c(2, 2, 1, 1))
varImpPlot(rf.strata, type = 1)

importanceOrder <- order(-rf.strata$importance[, "MeanDecreaseAccuracy"])
names <- rownames(rf.strata$importance)[importanceOrder][1:31]
par(mfrow = c(4, 8), mar = c(2, 2, 1, 1))
for (name in names) partialPlot(rf.strata, train.data, eval(name), main = name, 
                                xlab = name)

# view OOB-CV specificity and sensitiviy
plot(roc(rf.strata$votes[, 2], train.data$diagnosis))

# make test.data and remove random sample until both classes are equally prevalent
test.data.balanced <- test.data[-sample(which(test.data$diagnosis == 1))[1:1020], ]
table(test.data.balanced$diagnosis)
mean(test.data.balanced$diagnosis == predict(rf.strata, newdata = test.data.balanced))
...
x <- as.matrix(data.frame(xfactors, M.production.age, F.production.age, 
                          M.smoked.years, F.smoked.years))
library(glmnet)
glmmod <- glmnet(x, y = as.factor(diagnosis), alpha = 1, family = 'binomial')
...
cv.glmmod <- cv.glmnet(x, y = diagnosis, alpha = 1, type.measure = 'auc', 
                       family = "binomial", nfolds = 10)
cv.glmmod
plot(cv.glmmod)
lambda.min <- cv.glmmod$lambda.min
coef(cv.glmmod, s = "lambda.min")

pred <- predict(cv.glmmod, s = 'lambda.min', newx = x, type = "class")
table(diagnosis == pred)
mean(diagnosis == pred)
t <- diagnosis[which(diagnosis != pred)]
length(t)
table(t)
...
inTrain <- createDataPartition(y = data$diagnosis, p = .75, list = FALSE)
train.data <- mydata[inTrain, ]
test.data <- mydata[-inTrain, ]

table(train.data$diagnosis)
table(test.data$diagnosis)

ctrl <- trainControl(method = "repeatedcv", number = 10, savePredictions = TRUE)
mod_fit <- train(factor(diagnosis) ~ .,  data = train.data, method = "glm", 
                 family = "binomial", 
...
accuracy <- table(pred, test.data$diagnosis)
sum(diag(accuracy)) / sum(accuracy)
confusionMatrix(data = pred, test.data$diagnosis)

# 
mod_fit1 <- train(factor(diagnosis) ~ M.pregnancy.flu.time6 + M.pregnancy.flu.time3 + 
                    M.pregnancy.flu.time7 + production.mode1,  
                  data = train.data, method = "glm", family = "binomial", 
                  trControl = ctrl, tuneLength = 5)
summary(mod_fit1)

pred <- predict(mod_fit1, newdata = test.data)
accuracy <- table(pred, test.data$diagnosis)
sum(diag(accuracy)) / sum(accuracy)
confusionMatrix(data = pred, test.data$diagnosis)
...
CHD.bootYX3 <- bootplsglm(res, typeboot = "plsmodel", R = 100)
boxplots.bootpls(CHD.bootYX3, las = 2, mar = c(8, 2, 1, 1) + 0.1)
temp.ci <- confints.bootpls(CHD.bootYX3)
plots.confints.bootpls(temp.ci, typeIC = "BCa", colIC = c("blue", "blue", "blue", "blue"),
                       legendpos = "topright", las = 2, mar = c(5, 2, 1, 1) + 0.1)